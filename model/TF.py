# -*- coding: utf-8 -*-
"""
Created on Sun May 25 18:22:32 2025

@author: ChenPiQ
"""
from torch.nn import TransformerEncoderLayer,Parameter
...
class InterpretableTransformerEncoder(TransformerEncoderLayer):
    ...
    def _sa_block(self, x: Tensor,
                  attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor],is_causal: Optional[bool] ) -> Tensor:
...
class BrainNetworkTransformer(nn.Module):
    ...
    def __init__(self, roi_num = 100 ,pos_encoding = 'none',size =[360,100],pooling= [False, True],orthogonal=True,freeze_center =True,project_assignment=True):

        super().__init__()

        self.attention_list = nn.ModuleList()
        forward_dim = roi_num
        self.num_roi = roi_num
        self.pos_encoding = pos_encoding
        if self.pos_encoding == 'identity':
            self.node_identity = nn.Parameter(torch.zeros(
                self.num_roi, 360), requires_grad=True)
            forward_dim =  self.num_roi + 360
            nn.init.kaiming_normal_(self.node_identity)
        sizes = size
        sizes[0] = self.num_roi
        in_sizes = [self.num_roi] + sizes[:-1]
        do_pooling = pooling
